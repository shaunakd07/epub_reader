...
function applyMarginToContents(contents) {
  ...
  let styleEl = doc.getElementById("epub-margin-style");
  if (!styleEl) {
...
function applyMargin() {
  ...
  contents.forEach((c) => applyMarginToContents(c));
  scheduleRenditionResize();
}

function setLineReaderPosition(contents, top, height) {
  const doc = contents.document;
...
function applyLineReaderToContents(contents, enabled) {
  const doc = contents.document;
  ...
  let styleEl = doc.getElementById("line-reader-style");
  if (!styleEl && enabled) {
    styleEl = doc.createElement("style");
  ...
  let overlay = doc.getElementById("line-reader-overlay");
  if (!overlay && enabled) {
  ...
  if (enabled) {
    ...
    if (contents.__lineReaderHandler) {
      doc.removeEventListener("mousemove", contents.__lineReaderHandler);
      doc.removeEventListener("click", contents.__lineReaderHandler);
...
function applyLineReaderAll(enabled) {
  if (!rendition) {
    return;
  }
  const contents = rendition.getContents();
  contents.forEach((c) => applyLineReaderToContents(c, enabled));
}

function stepLineReader(direction) {
...
marginRange.addEventListener("input", (e) => {
  marginPx = Number(e.target.value);
